...
$stmt = $conn->prepare("INSERT INTO project_issues (project_id, issue_title, status, owner) VALUES (?, ?, ?, ?)");
$stmt->bind_param("isss", $pid, $title, $status, $owner);
if ($stmt->execute()) {
    ...
    // 3. 更新對應 project_id 的 projects 表格中的 issues_task 欄位
    // 假設 issues_task 欄位在 projects 表中
    ...
    if ($update_stmt->execute()) {
        ...
        echo "Failed to update ongoing issues count: " . $update_stmt->error;
    }
    ...
    http_response_code(500);
    echo "Failed to add new issue: " . $stmt->error;
}
